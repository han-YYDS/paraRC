...
// 用int表示byte偏移，其范围在 0 ~ 4MB
void CoorCommand::buildType41(int type, unsigned int ip, string blockname, int offset, int length, string method) {
  // set up corresponding parameters
  _type = type;
  _clientIp = ip;
  _blockName = blockname;
...
void CoorCommand::resolveType41() {
  // 2. ip
  _clientIp = readInt();
  // 3. filename
  _blockName = readString();
...
//         << ", filename: " << _filename << endl;
   ...
   cout << ", client: " << RedisUtil::ip2Str(_clientIp) << endl
        << ", blockname: " << _blockName << endl;
  } else if (_type == 41) {
   cout << ", client: " << RedisUtil::ip2Str(_clientIp) << endl
        << ", blockname: " << _blockName << endl
