...
void ECDAG::genECUnits()
{
    ...
    for (auto item : _ecNodeMap) // 出度 入度
    {
    ...
    while (leaves.getSize() > 0) // 层序遍历
    {
        ...
        for (auto pitem : parentNodes)// 遍历该节点的所有parentNode(生成节点)
        {
            ...
            {
                int tmpcnodeId = tmpChildIndices[tmpi]; // 该parent的某个child node
            ...
            // 尚未finish且已是全部leaves?
            if (!finished & allleaves)// out != 0 & in == 0
            {
                // generate ECUnit
                ...
                for (int tmpi = 0; tmpi < tmpChildIndices.size(); tmpi++)
                {
                    ...
                    outref[tmpcnodeId]--; // 修改出度
                }
                // decrease parent inref by childsize
                inref[pnodeId] -= tmpChildIndices.size(); // 修改入度
                if (inref[pnodeId] == 0)
                    leaves.push(pnodeId);
            }
        }
...
void ECDAG::genECCluster(unordered_map<int, int> coloring, int clustersize)
{
    ...
    while (leaves.size() > 0)
    {
        ...
        for (; starti < _ecUnitList.size(); starti++)
        {
            ...
            bool flag = true;
            for (auto curcid : curchildlist) // 如果该unit的child中不是叶子节点那么就退出
            {
...
{
    // 由于cluster存在切割,所以需要将parent color相同的cluster一起统计
    unordered_map<int, int> opmap; // color to cluster childs (由于之前的cluster定义为parent为相同color的合集)
                                    // 如果是自己传给自己,也会被记录
    for (int i = 0; i < _ecClusterList.size(); i++) // 针对于每个cluster
    {
        ...
        {
            int ccolor = coloring[item.first];// child color
                                   ...
                                   unordered_map<int, unsigned int> coloring_res)
{
    ...
    unordered_map<int, int> realleaves; // 非shorten
    ...
    for (int i = 0; i < _ecClusterList.size(); i++)
    {
        ...
        unordered_map<int, bool> childmap; // 标记 该cluster中的所有childs
        ...
        for (int j = 0; j < unitlist.size(); j++) // 遍历每个unit
        {
            ...
            for (auto cid : unitchilds)
            {
                if (childmap.find(cid) == childmap.end())
                {
                    ...
                    childlist.push_back(cid); // 只有一次,非重复出现
                }
            }
        }
        sort(childlist.begin(), childlist.end()); // 排序 根据idxID进行排序
        ...
        // 将unit中的coef 变为 cluster的coef
        for (int j = 0; j < unitlist.size(); j++) // 遍历unit
        {
            int unitId = unitlist[j];
            ECUnit *cunit = _ecUnitMap[unitId];
            // cout << "    unit: " << cunit->dump() << endl;
            ...
            vector<int> coeflist(childlist.size()); // cluster规模
            for (int tmpi = 0; tmpi < childlist.size(); tmpi++) // 遍历cluster每一个child
            {
                int cid = childlist[tmpi];
                int c = 0;
                for (int tmpj = 0; tmpj < unitchilds.size(); tmpj++) // 遍历该unit的child
                {
    ...
    // build fetchCompute 1 and Concact 2
    ...
    for (int i = 0; i < _ecClusterList.size(); i++)
    {
        ...
        vector<unsigned int> prevlocs; // 与childlist索引对应
        for (int tmpi = 0; tmpi < childlist.size(); tmpi++)
        {
            int cid = childlist[tmpi];
            unsigned int tmpl = coloring_res[cid];
            prevlocs.push_back(tmpl); // 含重复?
        }
        ...
        else
        {
            ...
            vector<vector<int>> coeflist; // 关于每个parent的coeflist
            for (int tmpi = 0; tmpi < parentlist.size(); tmpi++)
            {
                int pid = parentlist[tmpi];
                vector<int> coefs = coefmap[pid];
                coeflist.push_back(coefs); // 根据parentlist的映射填装coeflist
            }
...
void ECDAG::genConvECTasks2(vector<ECTask *> &tasklist,
                            int ecn, int eck, int ecw, int blkbytes, int pktbytes,
                            string stripename, vector<string> blocklist,
                            vector<unsigned int> loclist, int repairIdx)
{
    ...
    // 4. create fetch and compute
    ECTask *fetchComputeTask = new ECTask();
    fetchComputeTask->buildFetchCompute3(8, loclist[repairIdx], ip2cidlist, ctlist,
                                         stripename, cid2refs, ecw, blkbytes, pktbytes, blocklist[repairIdx]);
    tasklist.push_back(fetchComputeTask);
...
void ECDAG::genConvECTasksWithOffset(vector<ECTask *> &tasklist,
                                     int ecn, int eck, int ecw, int blkbytes, int pktbytes,
                                     string stripename, vector<string> blocklist,
                                     vector<unsigned int> loclist, int repairIdx, int offset)
{
    cout << "ECDAG::genConvECTasksWithOffset" << endl;
    ...
    // 3. repair by transfer
    // note: as we do not test miser for readblock, we should not get there
    if (ip2transferunits.size() > 0)
    {
        ...
        for (auto item : ip2transferunits)
        {
            ...
            ECTask *readComputeTask = new ECTask();
            readComputeTask->buildReadComputeWithOffset(7, ip, blockname, blkbytes, pktbytes, readIdxList,
                                                        ecw, stripename, ctlist, cacherefs, offset);
    ...
    for (auto item : readbid2cids)
    {
        ...
        ECTask *readtask = new ECTask();
        readtask->buildReadDiskWithOffset(6, loclist[bid], blocklist[bid],
                                          blkbytes, pktbytes, ecw, cid2refs, stripename, offset);
        tasklist.push_back(readtask);
