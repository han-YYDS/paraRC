  ...
  while (true) {
    ...
    if (rReply -> type == REDIS_REPLY_NIL) {
      ...
      switch (type) {
        ...
        case 41: degradeRead1(coorCmd); break;
        default: break;
      }
      delete coorCmd;
    }
...
void Coordinator::degradeRead1(CoorCommand* coorCmd) {
    struct timeval time1, time2, time3;
    ...
    if (method == "dist") {
        if (code == "Clay")
            repairBlockDist1_1(blockName, offset, length, clientIp, true, false);
        else
...
void Coordinator::repairBlockConv2(string blockName, unsigned int clientip, bool enforceip, bool wait) {
    cout << "Coor::repairBlockConv:blockName: " << blockName << endl;
    struct timeval time1, time2, time3;
    gettimeofday(&time1, NULL);
    ...
    vector<ECTask*> tasklist;
    //ecdag->genConvECTasks(tasklist, ecn, eck, ecw, blkbytes, pktbytes, stripename, blocklist, loclist, repairBlockIdx);
    ecdag->genConvECTasks2(tasklist, ecn, eck, ecw, blkbytes, pktbytes, stripename, blocklist, loclist, repairBlockIdx);
...
void Coordinator::repairBlockDist1_1(string blockName, int offset, int length,unsigned int clientip, bool enforceip, bool wait) {
    struct timeval time1, time2, time3;
    gettimeofday(&time1, NULL);
    ...
    // 4. prepare availidx
    vector<int> availIndex;
    int from = offset / subpktbytes;
    int to = (offset+length-1) / subpktbytes;
    vector<int> toRepairIndex;

    for (int i=0; i<ecn; i++) {
        for (int j=0; j<ecw; j++) {
            int pktidx = i*ecw+j;
            if (i == repairBlockIdx && j >= from && j <= to)
                toRepairIndex.push_back(pktidx);
            else
                availIndex.push_back(pktidx);
        }
    ...
    ECBase* ec = stripemeta->createECClass();
    ecw = ec->_w;
    ECDAG* ecdag1 = ec->Decode(availIndex, toRepairIndex);
    ECDAG* ecdag = ecdag1->split_1(toRepairIndex); 
    ecdag->Concact(toRepairIndex);
